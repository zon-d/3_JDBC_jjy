...
public class BoardView {
	...
	private void subBoardMenu(Board board) {
		...
		try {
			...
			switch (input) {
			case 1:
				insertComment(board.getBoardNo(), memberNo);
				break;
			case 2:
				updateComment(board.getCommentList(), memberNo);
				break;
			case 3:
				deleteComment(board.getCommentList(), memberNo);
				break;
			case 0:
				System.out.println("\n[게시판 메뉴로 돌아갑니다...]\n");
				break;
			...
			// 댓글 등록, 수정, 삭제 선택 시
			// 각각의 서비스 메서드 종료 후 다시서브메뉴 메서드 호출
			if (input > 0 && input < 5) {
			...
			for (Comment c : commentList) {
				...
				if (c.getCommentNo() == commentNo) { // 댓글 번호 일치
					...
					if (c.getMemberNo() == memberNo) { // 회원 번호 일치
						...
						if (ch == 'Y') {
							int result = cService.delelctComment(commentNo);
		...
		try {
			...
			if (result > 0) {
				...
				if (b != null) {
					System.out.println("--------------------------------------------------------");
					System.out.printf("글번호 : %d \n제목 : %s\n", b.getBoardNo(), b.getBoardTitle());
					System.out.printf("작성자 : %s | 작성일 : %s  \n조회수 : %d\n", b.getMemberName(), b.getCreateDate(),
							b.getReadCount());
					System.out.println("--------------------------------------------------------\n");
